...
import { api, type CheckoutStatus } from "@/lib/api";
import { Card } from "./ui/Card";

// ---------------------------------------------------------------------------
// "Verifying payment…" overlay for the dashboard.
//
// The browser cannot receive Dodo webhooks, so after a checkout the purchase
// row is advanced server-side by payment.* events and this polls it until it
// reaches a terminal state:
//   active            → success
//   failed/cancelled  → failure
//
// There is no timed-out second screen: if the webhook never arrives the
// overlay keeps waiting quietly and only says so. Escape dismisses it, which
// the dashboard reports as an undetermined result rather than a failure.
// ---------------------------------------------------------------------------
...
export function PaymentStatus({ checkoutId, onResolved }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-ink-900/50 px-4 backdrop-blur-sm">
      <Card className="w-full max-w-sm p-6 text-center animate-pop-in">
        ...
        <p className="mt-4 text-xs text-ink-400">Press Esc to keep browsing while it settles.</p>
      </Card>
    </div>
  );
}
